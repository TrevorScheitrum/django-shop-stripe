...
class StripeBackend(object):
    ...
    def stripe_payment_view(self, request):
        if request.POST:
            ...
            stripeCharge = stripe.Charge.create(**stripe_dict)
            
            transaction_id = stripeCharge.id

            self.shop.confirm_payment(self.shop.get_order_for_id(order_id), amount, transaction_id, self.backend_name) #, transaction_id, self.backend_name)
            ...
            #    request.user.save()
            return redirect('/shop/checkout/thank_you')
